...
def morp_per_col_on_tensor(image):
    ...
    morp_list_channel = [morp_per_col_on_mat(channel) for channel in list_channel]

    squeezed_list_channel = [np.squeeze(channel) for channel in morp_list_channel]
...
# LINE DETECTION
def locate_maxima_and_inverse_FHT(
    edge_image, hough, angleRange, threshold_abs=None, threshold_rel=None
):
    ...
    for i, coord in enumerate(coordinates):
        lines[i] = cv2.ximgproc.HoughPoint2Line(
            coord[::-1], edge_image, angleRange=angleRange
        )
...
def horizontal_line_detection(edge_image, angleRange, threshold_rel=None):
    ...
    hough = cv2.ximgproc.FastHoughTransform(
        edge_image, cv2.CV_32S, angleRange=angleRange
    )

    # locate local maxima and perform the inverse FHT
    coordinates, lines = locate_maxima_and_inverse_FHT(
        edge_image, hough, angleRange, None, threshold_rel
    )
...
def line_detection(image, horizontal_edges, vertical_edges):
    ...
    hor_hough, hor_coordinates, hor_lines = horizontal_line_detection(
        horizontal_edges, horizontal_angleRange, threshold_rel=horizontal_threshold
    )

    ver_hough, ver_coordinates, ver_lines = horizontal_line_detection(
        vertical_edges, vertical_angleRange, threshold_rel=horizontal_threshold
    )
...
def find_intersection_points(quad):
    ...
    u = (
        (line_2[:, :, 2] - line_2[:, :, 0]) * (line_1[:, :, 1] - line_2[:, :, 1])
...
def compute_score(gradient_map, four_points, eight_points):
    ...
    eight_points = (
        np.transpose(eight_points, axes=(0, 1, 3, 2)).round().astype(np.int64)
    )
    ...
    for i in indices:
        ...
        for j in range(4):
            ...
            inside_edge = np.array(draw.line(*inside_quad[:, [j - 1, j]].T.flatten()))
            ...
            total_interval = np.append(
                left_interval[:, 1:], right_interval[:, 1:], axis=1
            )
...
def step_3_1(gradient_map, hor_lines, ver_lines):
    ...
    pair_idx = np.array(
        np.meshgrid(np.arange(len(hor_pair)), np.arange(len(ver_pair)))
    ).T.reshape(-1, 2)

    quad = np.concatenate(
        (hor_pair[pair_idx[:, 0]], ver_pair[pair_idx[:, 1]]), axis=1
    ).astype(np.int64)
...
def step_3_2(resized_image, quads, contour_score):
    ...
    on_side = np.array([[77, 186], [163, 186], [163, 240], [77, 240]])
    ...
    for i in range(quads.shape[0]):
        ...
        tform = transform.estimate_transform("projective", quad, np.array(on_side))
        ...
        contrast_score[i] = np.sum(
            (inner_hist - outer_hist) ** 2 / (inner_hist + outer_hist + 1e-5)
        )
...
def detector(input_dir):
    ...
    for iter_, image_dir in enumerate(image_list):
        ...
        # 2. LINE DETECTION
        # width, height order
        hor_lines, ver_lines = line_detection(
            resized_image, horizontal_edges, vertical_edges
        )
